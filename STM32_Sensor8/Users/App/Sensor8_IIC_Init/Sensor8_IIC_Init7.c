...
void Sensor8_IIC7_Pin_Init(void)
{
    ...
    // 设置管脚为开漏模式
    GPIO_Pin_Config(Sensor8_IIC7_GPIO_RCC, Sensor8_IIC7_SDA|Sensor8_IIC7_SCL, GPIO_Mode_Out_OD, GPIO_Speed_50MHz, Sensor8_GPIO7);   // SDA|A5脚 SCL|A4脚 开漏输出 速率50MHz 
...
{
    Sensor8_GPIO7->CRL &= 0x0FFFFFFF;									// B组管脚 PIN7[31:28]
    Sensor8_GPIO7->CRL |= 8 << 28;										// 设为上拉输入模式
}
...
{
     Sensor8_GPIO7->CRL &= 0x0FFFFFFF;									// B组管脚 PIN7[31:28]
   	 Sensor8_GPIO7->CRL |= 3 << 28;										// 设为推挽输出模式 
}
