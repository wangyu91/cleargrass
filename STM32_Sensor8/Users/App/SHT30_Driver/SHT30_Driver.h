...
extern  u8 SHT30_Read_SN(u8 *ucSN);                                     // ¶ÁÈ¡SN
extern  u8 SHT30_Soft_Reset(void);                                      // Èí¸´Î»
extern  u8 SHT30_Get_Data(float *fTemp, float *fHumi);            	    // »ñÈ¡ÎÂÊª¶È

extern  u8 SHT30_IIC_Get_Data(SW_IIC_t *IIC_s, float *fTemp, float *fHumi);			// ¶ÁIICÊý¾Ý

extern  u8 SHT30_IIC_Read_Register(SW_IIC_t* IIC_s, u8 Chip_Addr, u16 usRead_Addr, u8* pBuffer, u16 usRead_Len);// ¶Á¼Ä´æÆ÷
extern  u8 SHT30_IIC_Write_Register(u8 usWrite_Addr, u8* pBuffer, u8 usWrite_Len);// Ð´¼Ä´æÆ÷
